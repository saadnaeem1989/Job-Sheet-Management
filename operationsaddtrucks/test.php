...
<!-- <meta charset="utf-8">
 ...
 <?php
//Taking job sheet id getting the time link of that jobheet and assigning to a hidden field
 ...
 $query="SELECT timelink FROM jobsheets WHERE id='$jobsheetid'";
 $result = $conn->query($query);
 if (!$result) die ("Database acess failed: " .$conn->error);
 $row = $result->fetch_array(MYSQLI_ASSOC);
 $timelink=$row['timelink'];
 
//  echo $_POST['jobsheetid'];
 // echo $totalunitswashed;
  ?>
 
 
<input type="hidden" name="timelink" value="<?php echo $timelink?>">
 <input type="hidden" name="totalunitswashed" value="<?php echo $totalunitswashed?>"> 
  ...
  <div class="col-xs-2">     
     ...
     <input type="text" id="t2reg<?php echo $i;?>" name="t2reg<?php echo $i;?>" class="form-control clear" placeholder="Rego">
      ...
      <br>
        ...
        <button type="submit" class="btn btn-block btn-warning" id="btnSubmit">
 <span class="glyphicon glyphicon-user"></span> Submit
</button>

<a href="../operations-jobsheets.php" class="btn btn-block btn-success"><span class="glyphicon glyphicon-arrow-left"></span> Back to Job Sheet Menu</a>
